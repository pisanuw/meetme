...
(async () => {
  ...
  function bindUi() {
    ...
    document
      .getElementById("meetings-pagination")
      .addEventListener("click", onMeetingsPaginationClick);
  ...
  async function loadUsers() {
    ...
    const { ok: usersOk, data: usersData } = await apiFetch(
  ...
  function renderUsers(users) {
    ...
    users.forEach((u) => {
      ...
      if (!u.is_super_admin) {
        const toggleAdminBtn = createButton(
          "admin-toggle",
  ...
  async function viewUserDetail(email) {
    ...
    const { ok: detailOk, data: detailData } = await apiFetch(
