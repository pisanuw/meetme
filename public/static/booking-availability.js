...
function updateAvailabilityModeHelp() {
  if (currentAvailabilityMode() === "specific_dates") {
    availabilityModeHelp.textContent =
      "Specific dates mode uses the date range columns below; click slots on exact dates and times.";
    return;
  }
...
async function loadInitialData() {
  ...
  if (!availabilityTimezoneInput.value)
    availabilityTimezoneInput.value = userProfileTimezone || "UTC";
  return true;
}

async function loadAndRenderExistingAvailability(eventTypeId) {
  const windowsRes = await apiFetch(
    `/api/bookings/availability?event_type_id=${encodeURIComponent(eventTypeId)}`
  );
  if (!windowsRes.ok) {
