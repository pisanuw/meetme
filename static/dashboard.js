...
(async () => {
  ...
  function renderMeetings(containerId, meetings, isOwner) {
    ...
    for (const m of meetings) {
      ...
      if (m.is_finalized) {
        ...
        finInfo.innerHTML = `📅 <strong>${escapeHtml(m.finalized_date || "")}</strong> at <strong>${escapeHtml(m.finalized_slot || "")}</strong> (${Number(m.duration_minutes)} min)`;
      ...
      if (m.is_finalized || isOwner) {
        btnView.textContent = "View";
      } else if (m.user_has_responded) {
        btnView.textContent = "Update Availability";
      } else {
        btnView.textContent = "Add Availability";
      }
  ...
  function renderBookingLinks(eventTypes, hostSlug) {
    ...
    grid.querySelectorAll(".js-delete-link").forEach((btn) => {
      btn.addEventListener("click", async () => {
        if (!confirm("Are you sure you want to delete this booking link? This cannot be undone."))
          return;
        const eventId = btn.dataset.eventId;
        btn.disabled = true;
        btn.textContent = "Deleting...";
        const resp = await apiFetch(
          `/api/bookings/event-types/${encodeURIComponent(eventId)}/delete`,
          { method: "POST" }
        );
        if (resp.ok) {
  ...
  async function sendReminders() {
    ...
    if (details.length > visibleDetails.length) {
      visibleDetails.push(
        `+${details.length - visibleDetails.length} more reminder${details.length - visibleDetails.length === 1 ? "" : "s"}`
      );
    }
  ...
  if (meetingsRes.ok) {
    ...
    showFlash(
      meetingsRes.data.error || `Failed to load meetings (HTTP ${meetingsRes.status}).`,
      "danger"
    );
  ...
  document.addEventListener("click", async (e) => {
    ...
    if (bookingBtn && bookingBtn.dataset.bookingAction === "cancel") {
      ...
      const mergeCancelled = (list) =>
        list.map((b) =>
          b.id === bookingId ? { ...b, ...cancelledBooking, status: "cancelled" } : b
        );
