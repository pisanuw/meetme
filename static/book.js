...
function renderWeekSlots(dayResults) {
  ...
  const firstAvailableDate =
    dayResults.find(({ slots }) => slots.length)?.date || dayResults[0]?.date || "";
...
(async () => {
  ...
  eventSelect.innerHTML = eventTypes
    .map((et) => {
      ...
      return `<option value="${escapeHtml(et.id)}" ${selected}>${escapeHtml(et.title)} (${Number(et.duration_minutes)}m)</option>`;
    })
    .join("");
