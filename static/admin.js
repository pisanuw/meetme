...
(async () => {
  ...
  function renderStats(statsData) {
    ...
    document.getElementById("stat-meetings").textContent = statsData.total_meetings ?? "—";
  ...
  async function loadUsers() {
    ...
    const { ok: usersOk, data: usersData } = await apiFetch(`/api/admin/users?${params.toString()}`);
    if (!usersOk) {
      showFlash(usersData.error || "Failed to load users.", "danger");
      return;
    }
    allUsers = usersData.users || [];
    usersPagination = usersData.pagination || null;
  ...
  async function toggleAdminRole(email, makeAdmin) {
    ...
    const { ok: toggleOk, data: toggleData } = await apiFetch("/api/admin/users/admin", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email, is_admin: makeAdmin }),
    });
    if (!toggleOk) {
      showFlash(toggleData.error || "Failed to update admin role.", "danger");
      return;
    }
  ...
  async function impersonateUser(email) {
    if (!confirm(`Act as ${email}? You can return to your admin account from the top navigation.`))
      return;
    const { ok: impOk, data: impData } = await apiFetch("/api/admin/impersonate", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email }),
    });
    if (!impOk) {
      showFlash(impData.error || "Failed to start impersonation.", "danger");
      return;
    }
  ...
  function openUserModal(email) {
    editingEmail = email;
    const u = email ? allUsers.find((usr) => usr.email === email) : null;
  ...
  async function saveUser(e) {
    ...
    const { ok: saveOk, status: saveStatus, data: saveData } = await apiFetch("/api/admin/users", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body),
    });
    if (saveOk) {
      showFlash(saveStatus === 201 ? "User created." : "User updated.", "success");
      closeUserModal();
      await loadUsers();
    } else {
      showFlash(saveData.error || "Failed to save user.", "danger");
    }
  }
  ...
  async function deleteUser() {
    if (!editingEmail || !confirm(`Permanently delete ${editingEmail}? This cannot be undone.`))
      return;
    const { ok: delOk, data: delData } = await apiFetch("/api/admin/users/delete", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    if (delOk) {
      ...
      showFlash(delData.error || "Failed to delete user.", "danger");
    }
  }
  ...
  async function viewUserDetail(email) {
    ...
    const { ok: detailOk, data: detailData } = await apiFetch(`/api/admin/users/${encodeURIComponent(email)}`);
    if (!detailOk) {
      document.getElementById("detail-body").innerHTML =
        `<p class="text-muted">${escapeHtml(detailData.error || "Failed to load.")}</p>`;
      return;
    }
    ...
    const addRow = (lbl, val) => {
      ...
      th.className = "admin-detail-label";
      th.textContent = lbl;
    ...
    addRow("Meetings created", detailData.created_meetings?.length ?? 0);
    addRow("Meetings invited", detailData.invited_meetings?.length ?? 0);
    detailBody.appendChild(table);

    if (detailData.created_meetings?.length) {
      ...
      detailData.created_meetings.forEach((m) => {
  ...
  async function loadMeetings() {
    ...
    const { ok: mtgOk, data: mtgData } = await apiFetch(`/api/admin/meetings?${params.toString()}`);
    if (!mtgOk) {
      tbody.innerHTML = `<tr><td colspan="7">${escapeHtml(mtgData.error || "Failed")}</td></tr>`;
      return;
    }
    const meetings = mtgData.meetings || [];
    meetingsPagination = mtgData.pagination || null;
  ...
  async function loadEvents() {
    ...
    const { ok: evOk, data: evData } = await apiFetch("/api/admin/events");
    if (!evOk) {
      tbody.innerHTML = `<tr><td colspan="5">${escapeHtml(evData.error || "Failed")}</td></tr>`;
      return;
    }
  ...
  function renderPagination(containerId, pagination, kind) {
    ...
    if (!container) return;
    container.innerHTML = "";
    if (!pagination || pagination.total <= pagination.page_size) return;
