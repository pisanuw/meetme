...
function looksLikeToken(value) {
  return /^[A-Za-z0-9]{12,64}$/.test(value) && !value.includes("@");
}
...
if (emailInput && submitBtn) {
  emailInput.addEventListener("input", () => {
    ...
    if (looksLikeToken(value)) {
      submitBtn.textContent = "Log in with token";
    } else {
...
if (form) {
  form.addEventListener("submit", async (e) => {
    ...
    if (looksLikeToken(value)) {
      window.location.href = "/api/auth/magic-link/verify?token=" + encodeURIComponent(value);
      return;
    }
    ...
    if (ok) {
      window.location.href = "/email-sent.html?email=" + encodeURIComponent(value);
    } else {
