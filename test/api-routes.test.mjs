...
test("auth email preferences get and update for signed-in user", async () => {
  ...
  const prefsInDb = await store("email_preferences").get("email:prefs@example.com", {
    type: "json",
  });
...
test("auth magic-link verify sets session cookie", async () => {
  const token = "opaque-verify-01";
  await store("login_tokens").setJSON(token, {
  ...
  const req = new Request(
    `http://localhost:8888/api/auth/magic-link/verify?token=${encodeURIComponent(token)}`,
    { method: "GET" }
  );
...
test("meeting detail redacts all_invites emails for non-creators", async () => {
  const creator = {
    id: "u-creator-privacy",
    email: "creator-privacy@example.com",
    name: "Creator",
  };
  ...
  assert.equal(
    creatorBody.all_invites.some((inv) => inv.email === "friend@example.com"),
    true
  );
});
...
test("email preferences confirm and apply global opt-out", async () => {
  ...
  const prefs = await store("email_preferences").get("email:recipient@example.com", {
    type: "json",
  });
...
test("meetings handler delegates action subroutes when availability hits /api/meetings/*", async () => {
  ...
  const availabilityRows = await store("availability").get(`meeting:${meetingId}`, {
    type: "json",
  });
