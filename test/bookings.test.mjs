...
test("event types default and persist booking day time ranges", async () => {
  ...
  const defaultPrivate = privateListBody.event_types.find(
    (item) => item.id === defaultEventBody.event_type.id
  );
  const customPrivate = privateListBody.event_types.find(
    (item) => item.id === customEventBody.event_type.id
  );
  ...
  const defaultPublic = publicPageBody.event_types.find(
    (item) => item.id === defaultEventBody.event_type.id
  );
  const customPublic = publicPageBody.event_types.find(
    (item) => item.id === customEventBody.event_type.id
  );
...
test("specific-dates availability mode only returns slots on configured dates", async () => {
  ...
  const setAvailReq = makeJsonRequest("http://localhost:8888/api/bookings/availability", {
    ...
    body: {
      ...
      start_date: "2026-03-01",
      end_date: "2026-03-31",
      windows: [{ date: "2026-03-18", start_time: "09:00", end_time: "10:00", timezone: "UTC" }],
    },
  });
  const setAvailRes = await bookingsHandler(setAvailReq, {});
  assert.equal(setAvailRes.status, 200);
...
test("deleting an event type deletes bookings for that event type", async () => {
  const host = {
    id: "host-delete-et",
    email: "host-delete-et@example.com",
    name: "Host Delete ET",
  };
  const guest = {
    id: "guest-delete-et",
    email: "guest-delete-et@example.com",
    name: "Guest Delete ET",
  };
  await seedUser(host);
  await seedUser(guest);
  ...
  const deleteEventRes = await bookingsHandler(
    makeJsonRequest(
      `http://localhost:8888/api/bookings/event-types/${encodeURIComponent(eventTypeId)}/delete`,
      {
        method: "POST",
        headers: { cookie: authCookie(host) },
        body: {},
...
test("different event types can expose different availability schedules", async () => {
  const host = {
    id: "host-per-event",
    email: "host-per-event@example.com",
    name: "Host Per Event",
  };
  await seedUser(host);
  ...
  const socialAvailRes = await bookingsHandler(
    makeJsonRequest("http://localhost:8888/api/bookings/availability", {
      ...
      body: {
        ...
        end_date: "2026-03-31",
        windows: [
          { day_of_week: "Monday", start_time: "14:00", end_time: "17:00", timezone: "UTC" },
        ],
      },
    }),
    {}
  ...
  const pageBody = await responseJson(
    await bookingsHandler(
      new Request("http://localhost:8888/api/bookings/page/host-per-event", { method: "GET" }),
      {}
    )
  );
