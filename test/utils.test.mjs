...
test("encryptSecret fails closed when TOKEN_ENCRYPTION_KEY is unset", () => {
  const saved = process.env.TOKEN_ENCRYPTION_KEY;
  delete process.env.TOKEN_ENCRYPTION_KEY;
  try {
    assert.throws(() => encryptSecret("super-secret-token"), /TOKEN_ENCRYPTION_KEY/);
  } finally {
    process.env.TOKEN_ENCRYPTION_KEY = saved;
  }
});

test("encryptSecret rejects a too-weak TOKEN_ENCRYPTION_KEY", () => {
  const saved = process.env.TOKEN_ENCRYPTION_KEY;
  process.env.TOKEN_ENCRYPTION_KEY = "short";
  try {
    assert.throws(() => encryptSecret("super-secret-token"), /too weak/);
  } finally {
    process.env.TOKEN_ENCRYPTION_KEY = saved;
  }
});
...
test("checkRateLimit fails closed when the backing store errors", async () => {
  setDbFactoryForTests(() => ({
  ...
  try {
    ...
    assert.equal(denied.ok, false, "default must deny when the store is unavailable");
    const allowed = await checkRateLimit({ ...opts, failOpen: true });
    assert.equal(allowed.ok, true, "failOpen:true must allow when the store is unavailable");
  } finally {
    clearDbFactoryForTests();
  }
});
