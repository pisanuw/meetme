...
export function validateEventTypeBody(body, authUser, currentUser, existingIds = []) {
  ...
  if (title.length > LIMITS.BOOKING_EVENT_TITLE_MAX) {
    return {
      error: {
        status: 400,
        message: `Event title must be ${LIMITS.BOOKING_EVENT_TITLE_MAX} characters or fewer.`,
      },
    };
  }
  if (description.length > LIMITS.BOOKING_EVENT_DESCRIPTION_MAX) {
    return {
      error: {
        status: 400,
        message: `Description must be ${LIMITS.BOOKING_EVENT_DESCRIPTION_MAX} characters or fewer.`,
      },
    };
  }
  ...
  if (
    ...
    return {
      error: {
        status: 400,
        message: `duration_minutes must be between ${LIMITS.DURATION_MIN} and ${LIMITS.BOOKING_DURATION_MAX}.`,
      },
    };
  }
  if (
    ...
    return {
      error: {
        status: 400,
        message: `group_capacity must be between 1 and ${LIMITS.BOOKING_GROUP_CAPACITY_MAX}.`,
      },
    };
  }
  if (!eventTypeId && existingIds.length >= LIMITS.BOOKING_EVENT_TYPES_MAX) {
    return {
      error: {
        status: 400,
        message: `You can create at most ${LIMITS.BOOKING_EVENT_TYPES_MAX} event types.`,
      },
    };
  }
...
export function validateAvailabilityBody(body, eventType) {
  ...
  if ((startDate && !endDate) || (!startDate && endDate)) {
    return {
      error: {
        status: 400,
        message: "start_date and end_date must both be provided when using a date range.",
      },
    };
  }
  if (startDate && (!isValidDate(startDate) || !isValidDate(endDate))) {
    return {
      error: { status: 400, message: "start_date and end_date must be in YYYY-MM-DD format." },
    };
  }
  ...
  if (windows.length > LIMITS.BOOKING_AVAIL_WINDOWS_MAX) {
    return {
      error: {
        status: 400,
        message: `You can set at most ${LIMITS.BOOKING_AVAIL_WINDOWS_MAX} availability windows.`,
      },
    };
  }
    ...
    if (mode === "specific_dates") {
      ...
      if (!isValidDate(date)) {
        return {
          error: {
            status: 400,
            message: "Each availability row must include a valid date in YYYY-MM-DD format.",
          },
        };
      }
      if (!isDateInRange(date, startDate, endDate)) {
        return {
          error: {
            status: 400,
            message: "Availability date must be within the selected date range.",
          },
        };
      }
      normalized.push({
