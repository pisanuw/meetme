...
import { loadAvailabilityConfig, buildSlotsResponse } from "./lib/bookings-availability.mjs";
import {
...
async function handleBookings(req, _context) {
  ...
  if (req.method === "POST" && deleteEventMatch) {
    ...
    try {
      ...
      log("info", FN, "cascade deleted bookings for event type", {
        eventTypeId,
        deleted_bookings: deletedBookings,
      });
    } catch (cleanupErr) {
      log("warn", FN, "cleanup failed after event type delete", {
        eventTypeId,
  ...
  if (req.method === "POST" && bookMatch) {
    ...
    const postWriteSlotIds = asArray(
      await bookingsDb.get(slotKey, { type: "json" }).catch(() => [])
    );
    let postWriteConfirmedCount = 0;
    for (const candidateId of postWriteSlotIds) {
      const candidate = await bookingsDb
        .get(`booking:${candidateId}`, { type: "json" })
        .catch(() => null);
    ...
    if (postWriteConfirmedCount > (eventType.group_capacity || 1)) {
      ...
      await bookingsDb
        .setJSON(
          slotKey,
          postWriteSlotIds.filter((id) => id !== bookingId)
        )
        .catch(() => null);
      return errorResponse(409, "Selected slot is no longer available.");
    }
