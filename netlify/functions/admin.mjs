...
async function handleAdmin(req, context) {
  ...
  let dbUser;
  try {
    dbUser = await usersDb.get(tokenUser.email, { type: "json" });
  } catch {
    // Store error: super-admins (ADMIN_EMAILS) pass through; DB-based admins cannot
    // be verified, so fail closed to prevent stale revoked-admin JWT access.
    if (!isSuperAdminEmail(tokenUser.email)) {
      return errorResponse(503, "Admin status could not be verified. Please try again.");
    }
  }
  ...
  if (req.method === "GET" && userPathMatch) {
    ...
    const meetingDetails = await Promise.all(
      index.map(async (mid) => {
        const [m, rawInvites] = await Promise.all([
          getMeetingRecord(meetings, mid),
          invites.get(`meeting:${mid}`, { type: "json" }).catch(() => []),
        ]);
        if (!m) return null;
        const meetingInvites = asArray(rawInvites);
        const inv = meetingInvites.find((i) => i.email === email);
        const needsAvail = m.creator_id === u.id || !!inv;
        const avList = needsAvail
          ? asArray(await availability.get(`meeting:${mid}`, { type: "json" }).catch(() => []))
          : [];
        return { m, meetingInvites, inv, avList };
      })
    );

    for (const detail of meetingDetails) {
      if (!detail) continue;
      const { m, meetingInvites, inv, avList } = detail;
  ...
  if (req.method === "POST" && path === "users/admin") {
    ...
    const isSuper = isSuperAdminEmail(savedTarget.email);
    return jsonResponse(200, {
      success: true,
      user: {
        ...sanitizeUser(savedTarget),
        is_super_admin: isSuper,
        is_admin: isSuper || !!savedTarget.is_admin,
      },
    });
  }
