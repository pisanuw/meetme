...
async function sendMagicLinkEmail(email, link, token) {
  const result = await sendEmail({
    ...
    html: `
      ...
      <p>Or enter this token on the login page:</p>
      <p style="font-family:monospace;font-size:18px;letter-spacing:1px;"><strong>${token}</strong></p>
      <p>This link and token expire in 15 minutes and can only be used once.</p>
    `,
    text: `Sign in to MeetMe: ${link}\n\nOr enter this token on the login page: ${token}\n\nThis link and token expire in 15 minutes and can only be used once.`,
...
export default async function handleMagicLink(req, context) {
  ...
  if (req.method === "GET" && path === "magic-link/verify") {
    ...
    if (isRateLimitEnabled()) {
      ...
      const rl = await checkRateLimit({
        bucket: "auth_magic_link_verify_ip",
        key: ip,
        limit: 20,
        windowMs: 15 * 60 * 1000,
      });
      if (!rl.ok) {
        log("warn", FN, "magic-link/verify rate limited", { ip });
        return redirectResponse("/?error=too-many-attempts");
      }
    }
  ...
  if (req.method === "POST" && path === "magic-link/request") {
    ...
    const sendResult = await sendMagicLinkEmail(emailKey, link, token);
    if (!sendResult.ok) {
